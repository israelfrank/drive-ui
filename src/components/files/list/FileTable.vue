  ...
  <div>
    ...
    >
      <template v-slot:item="{item,isSelected,select}">
        ...
        >
          <td>
            <v-simple-checkbox color="#5f7286" v-ripple :value="isSelected" @input="select($event)"></v-simple-checkbox>
          </td>
          <td id="file-icon">
      ...
      <template v-slot:header.data-table-select="{props,on}">
        <v-simple-checkbox color="#5f7286" v-ripple v-bind="props" v-on="on"></v-simple-checkbox>
      </template>
      <template v-slot:no-data>{{$t('NoData')}}</template>
    </v-data-table>
    <v-row justify="center">
      <v-pagination id="pagination" color="#5f7286" v-model="page" :length="pageCount"></v-pagination>
    </v-row>
