  ...
  <v-menu bottom offset-y max-height="400" id="load-menu" min-width="300" v-model="files.length">
    <template v-slot:activator="{ on, attrs }">
      <v-btn v-bind="attrs" v-on="on" text id="show-menu-button">
        <v-progress-circular id="loading" :size="40" :width="3" color="#88bcd9" indeterminate></v-progress-circular>
        <p>{{$t('file.Loading')}}</p>
    ...
    <v-list>
      <v-list-item v-for="(file,index) in files" :key="index">
        <div id="file-container">
          ...
          <v-progress-linear :value="file.progress" color="#88bcd9"></v-progress-linear>
        </div>
